...
class RaceParticipantTest {
    ...
    fun raceParticipant_RaceStarted_ProgressUpdated() = runTest {
        ...
        advanceTimeBy(raceParticipant.progressDelayMillis) //これで時間を進められる
        runCurrent() //これで上で進めた分のところで保留されていたところを実行
    ...
    fun raceParticipant_RacePaused_ProgressUpdated() = runTest {
        ...
        runCurrent()
        racerJob.cancelAndJoin()//キャンセルし完了を待つ
        assertEquals(expectedProgress, raceParticipant.currentProgress)
    }
    ...
    @Test(expected = IllegalArgumentException::class) //例外の時
